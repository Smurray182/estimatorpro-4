...
export const MasterBathroomPaintSection: React.FC<SectionProps> = ({
  initialData,
  onUpdate,
}) => {
  ...
  const totalCost = useMemo(() => {
    ...
    if (details.wholeHome) cost += 3500;
  ...
  const handleToggleClick = (field: keyof PaintDetails) => {
    setDetails((prev) => ({
      ...prev,
      [field]: !prev[field as "smBath" | "lgBath" | "wholeHome"],
    }));
  };
      ...
      <VStack spacing={6} align="stretch">
        <HStack>
          ...
          <CustomToggleButton
            label="Whole Home"
            cost={3500}
            isSelected={details.wholeHome}
            onClick={() => handleToggleClick("wholeHome")}
          />
        </HStack>
        <CustomNumberInput
          label="Custom Area"
