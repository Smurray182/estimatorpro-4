...
export const MasterBathroomAccessoriesInstallSection: React.FC<
  SectionProps
> = ({ initialData, onUpdate }) => {
  const [details, setDetails] = useState<AccessoriesDetails>(
    initialData?.accessoriesInstallDetails || { laborHours: "" }
  );
  const totalCost = useMemo(
    () => (Number(details.laborHours) || 0) * 60,
    [details]
  );
