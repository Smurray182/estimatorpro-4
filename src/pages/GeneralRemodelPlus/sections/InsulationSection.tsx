...
// Parser function to read saved data from the description string
const parseInsulationAreas = (desc: string | undefined): InsulationArea[] => {
  if (!desc) return [];
  const areas: InsulationArea[] = [];
  const regex = /-\s*Insulation Area:\s*(\d+\.?\d*)\s*sq ft/g;
...
const InsulationSection: React.FC<PageProps> = ({
  estimateData,
  setEstimateData,
  remodelType,
}) => {
  ...
  // State for insulation areas
  const [areas, setAreas] = useState<InsulationArea[]>(() =>
    parseInsulationAreas(existingItem?.description)
  );
  const [sqft, setSqft] = useState("");

  useEffect(() => {
    const totalSqFt = areas.reduce((sum, area) => sum + area.sqft, 0);
    const totalCost = totalSqFt * 3; // $3 per sqft

    const buildDescription = () => {
      if (areas.length === 0) return "Insulation scope of work.";
      return areas.map((a) => `- Insulation Area: ${a.sqft} sq ft`).join("\n");
    };
